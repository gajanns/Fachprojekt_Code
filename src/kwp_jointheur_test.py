...
def work_kWP_cumulative(links, n, demands, ilp_method, setup, time_out, res_handler):
    ...
    try:
        ...
        for i,solution in enumerate(solutions):
            result_dict = dict()
            result_dict.update(setup)
            algorithm_name="kwp_jointheur_" + str(i)
...
def kwp_all_topologies_synthetic_demands():
    ...
    result_filename = os.path.join(OUT_DIR, "results_kwp_all_topologies.json")
...
def geant_all_algorithms():
    ...
    result_filename = os.path.join(OUT_DIR, "results_kwp_all_algorithms.json")
    result_handler = JsonResultWriter(result_filename, overwrite=True)
...
def kwp_real_demands():
    ...
    # setup result handler
    result_filename = os.path.join(OUT_DIR, "results_kwp_real_demands.json")
    result_handler = JsonResultWriter(result_filename, overwrite=True)
