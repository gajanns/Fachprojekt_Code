...
DIR_PLOT = utility.create_dirs(f"../out/plots/alg3")
...
def create_algo_c_map(plot_type:str):
    ...
    if plot_type.startswith("all_algorithms"):
        ...
        algo_map = {
            ...
            'GreedyWaypoints': "hotpink",
            'JointHeur': "seagreen",
            '1/2N-JointHeur': "mediumorchid",
            '1/3N-JointHeur': "goldenrod",
            '1/4N-JointHeur': "salmon",
            '1/5N-JointHeur': "grey",
        }
    return algo_map
...
def prepare_data_and_plot(df, title, plot_type):
    ...
    df = df[df["algorithm"] != "nodes_kwp_jointheur_1"]
    ...
    df["algorithm_complete"] = df["algorithm_complete"].str.replace("NodesKwpJointheur5", "1/5N-JointHeur")
    df["algorithm_complete"] = df["algorithm_complete"].str.replace("NodesKwpJointheur4", "1/4N-JointHeur")
    df["algorithm_complete"] = df["algorithm_complete"].str.replace("NodesKwpJointheur3", "1/3N-JointHeur")
    df["algorithm_complete"] = df["algorithm_complete"].str.replace("NodesKwpJointheur2", "1/2N-JointHeur")
    df["algorithm_complete"] = df["algorithm_complete"].str.replace("NodesKwpJointheur1", "1N-JointHeur")
...
def main():
    ...
    data_all_algorithm = os.path.join(dir_data, "results_nodes_kwp_all_algorithms.json")
    if os.path.exists(data_all_algorithm):
        ...
        print(f"{utility.FAIL}results_nodes_kwp_all_algorithms.json not existing in {dir_data}{utility.CEND}")

    # figure (all_topologies)
    data_all_topologies = os.path.join(dir_data, "results_nodes_kwp_all_topologies.json")
    if os.path.exists(data_all_topologies):
        ...
        print(f"{utility.FAIL}results_nodes_all_topologies.json not existing in {dir_data}{utility.CEND}")

    # figure (real_demands)
    data_real_demands = os.path.join(dir_data, "results_nodes_kwp_real_demands.json")
    if os.path.exists(data_real_demands):
        ...
        print(f"{utility.FAIL}results_nodes_real_demands.json not existing in {dir_data}{utility.CEND}")
